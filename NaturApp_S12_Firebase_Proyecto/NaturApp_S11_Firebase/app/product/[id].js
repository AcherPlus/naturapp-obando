...
export default function ProductDetailScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      // Si el producto viene en los parámetros, usarlo directamente
      if (productParam) {
        try {
          const p = JSON.parse(productParam);
          setProduct(p);
          setLoading(false);
          return;
        } catch (e) {
          console.error('Error parsing product param:', e);
        }
      }
      
      // Si no, obtenerlo por ID
      const p = await getProductById(id);
      setProduct(p);
      setLoading(false);
    };
    load();
  }, [id, productParam, getProductById]);
