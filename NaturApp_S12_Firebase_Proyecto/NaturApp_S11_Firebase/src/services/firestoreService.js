...
export const ProductService = {
  ...
  getAll: async (categoryFilter = null) => {
    try {
      ...
      // Si Firestore está vacío, usar fallback local
      if (results.length === 0) {
        console.log('ProductService.getAll: Firestore vacío, usando fallback local');
      ...
      console.log('ProductService.getAll error:', err.message);
      let data = LOCAL_PRODUCTS;
      if (categoryFilter) data = data.filter(p => p.category === categoryFilter);
      return data;
    }
...
export const CategoryService = {
  getAll: async () => {
    try {
      ...
      // Si Firestore está vacío, usar fallback local
      if (results.length === 0) {
        console.log('CategoryService.getAll: Firestore vacío, usando fallback local');
        return LOCAL_CATEGORIES;
      }
      
      return results;
    } catch (err) {
      console.log('CategoryService.getAll error:', err.message);
      return LOCAL_CATEGORIES;
    }
  },
};
...
export const CartService = {
  ...
  addItem: async (userId, item) => {
    try {
      ...
      const existing = localCart.items.find(i => i.id === item.id);
      if (existing) {
  ...
  updateQuantity: async (userId, productId, quantity) => {
    try {
      ...
      const item = localCart.items.find(i => i.id === id);
      if (item) {
        item.quantity = quantity;
        if (quantity <= 0) localCart.items = localCart.items.filter(i => i.id !== id);
      }
    }
  },
  ...
  removeItem: async (userId, productId) => {
    try {
      ...
      localCart.items = localCart.items.filter(i => i.id !== id);
    }
  },
